...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
    ...
    private void solution(SimpleTreeNode node) {
        if (node.right != null || node.left != null) {
            ...
            if ((Integer) node.value % 2 == 1) {
                node.right = new SimpleTreeNode((T) new Integer(-1));
            } else {
                node.left = new SimpleTreeNode((T) new Integer(1));
            }
        }
    }
